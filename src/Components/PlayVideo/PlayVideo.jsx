  ...
  return (
    <div className='play-video'>
        ...
        <div className="vid-description">
            <p>{apiData?apiData.snippet.description.slice(0,250):"Desc here"}</p>
            <hr />
            <h4>{apiData?value_converter(apiData.statistics.commentCount):"104"}</h4>
            {commentData.map((item,index)=>{
                return(
                    <div key={index} className="comment">
                        <img src={item.snippet.topLevelComment.snippet.authorProfileImageUrl} alt="" />
                    <div>
                        <h3>{item.snippet.topLevelComment.snippet.authorDisplayName} <span>1 sec ago</span></h3>
